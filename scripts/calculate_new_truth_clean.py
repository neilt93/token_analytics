...
def calculate_current_prices(data):
    ...
    for token, df in data.items():
        current_prices[token] = float(df['close'].iloc[-1])
...
def calculate_24h_changes(data):
    ...
    for token, df in data.items():
        if len(df) >= 2:
            ...
            changes[token] = float(change_pct)
    return changes
...
def calculate_average_prices(data):
    ...
    avg_prices = {}
    for token, df in data.items():
        avg_prices[token] = float(df['close'].mean())
    return avg_prices
...
def calculate_market_caps(data):
    ...
    for token, df in data.items():
        ...
        market_caps[token] = float(avg_price * avg_volume / 1e6)
    return market_caps
...
def calculate_volatility(data):
    ...
    for token, df in data.items():
        ...
        volatility[token] = {
            ...
            'range': float((df['close'].max() - df['close'].min()) / df['close'].min() * 100)
        }
    return volatility
...
def calculate_truth_values():
    ...
    for query in updated_queries['queries']:
        ...
        # Single token hard queries
            ...
            if len(june_data) > 0:
                higher_days = int((june_data['close'] > june_data['open']).sum())
        ...
        # Multi-token hard queries
            ...
            eth_returns = float(eth_data['close'].pct_change().dropna().mean() * 100)
            sol_returns = float(sol_data['close'].pct_change().dropna().mean() * 100)
    ...
    with open('data/queries_new_with_truth_clean.yaml', 'w') as f:
    ...
    print(f"\n💾 Updated queries saved to: data/queries_new_with_truth_clean.yaml")
